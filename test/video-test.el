...
(ert-deftest video-source-uri-converts-local-file ()
  (let ((uri (video-source-uri (video-test--fixture))))
    (should (string-prefix-p "file:///" uri))
    (should (string-suffix-p "test.webm" uri))))

(ert-deftest video-source-uri-retains-network-uri ()
  (should (equal (video-source-uri "https://example.test/video.mp4")
                 "https://example.test/video.mp4")))

(ert-deftest video-source-headers-reject-invalid-fields ()
  (should
   (equal
    (video-source-header-vector
     '(("Referer" . "https://example.test/")
       ("User-Agent" . "video.el test")))
    ["Referer" "https://example.test/" "User-Agent" "video.el test"]))
  (should-error
   (video-source-header-vector '(("Bad Header" . "value"))))
  (should-error
   (video-source-header-vector '(("Referer" . "ok\nInjected: value"))))
  (should-error
   (video-source-header-vector
  ...
  (let ((file (make-temp-file "video-gif-")))
    (unwind-protect
        (dolist (case '((1 nil nil 1 nil)
          ...
          (should (equal (video-source-gif-metadata file)
        ...
        (progn
          (dotimes (length (length data))
            (with-temp-buffer
              ...
              (write-region (point-min) (point-max) file nil 'silent))
            (should-not (video-source-gif-metadata file)))
          (with-temp-buffer
...
(ert-deftest video-loop-respects-paused-eos-and-explicit-disable ()
  (let ((player (video--make-player :handle 'native :kind 'video
        ...
        (rewinds 0))
    (cl-letf (((symbol-function 'video-native-poll)
               (lambda (_) (list :state 'stopped :eos eos :seekable t)))
              ((symbol-function 'video-native-seek)
               (lambda (_ position) (should (= position 0))
                 (cl-incf rewinds) (setq eos nil)))
              ((symbol-function 'video-native-play) #'ignore)
              ((symbol-function 'video-native-pause) #'ignore)
              ((symbol-function 'video--show-player-controls) #'ignore)
              ((symbol-function 'video--update-player-buffering-animation) #'ignore))
      (video-player-set-loop player t)
      (video--dispatch player)
      (should (= rewinds 1))
      (should (eq (video-player-desired-state player) 'playing))
      (setq eos t)
      ...
      (should (= rewinds 1))
      (should (eq (video-player-desired-state player) 'paused))
      (video-player-set-loop player nil)
      (video-player-play player)
      (video--dispatch player)
      (should (= rewinds 1))
      (should (eq (video-player-desired-state player) 'paused)))))
...
(ert-deftest video-loop-toggle-overrides-animation-file-repetition ()
  (let ((player (video--make-player :handle 'native :kind 'image :animated-p t
    ...
    (cl-letf (((symbol-function 'video-native-poll)
      ...
      (should (eq (video-player-desired-state player) 'paused))
      (video-player-toggle-loop player)
      ;; Enabling repetition does not restart an exhausted paused animation.
      (video--dispatch player)
      (should (eq (video-player-desired-state player) 'paused))
      (should (video-player-loop-p player)))))
...
(ert-deftest video-target-host-close-is-detached-and-reentrant ()
  (let* ((player (video--make-player))
         (calls 0)
         attached-during-close
         (target (video--make-target
                  :player player
                  :close-function
                  (lambda (closed-target)
                    (cl-incf calls)
                    (setq attached-during-close
                          (memq closed-target (video-player-targets player)))
                    (video-target-close closed-target)))))
    (setf (video-player-targets player) (list target))
    (video-target-close target)
    (video-target-close target)
    (should-not attached-during-close)
    (should (= calls 1))))
...
(ert-deftest video-target-prepare-can-close-before-copy ()
  (let* ((player (video--make-player :kind 'image))
    ...
    (should (video-target-closed target))))
...
(ert-deftest video-frame-close-preserves-other-windows-and-repeat-open ()
  (skip-unless (and (display-graphic-p) (fboundp 'video-native-create)))
  (let ((source (generate-new-buffer " *video-frame-source*"))
        (viewer (generate-new-buffer " *video-frame-viewer*"))
        (other (generate-new-buffer " *video-frame-other*"))
        detached quit-called)
    (unwind-protect
        (save-window-excursion
          (delete-other-windows)
          (switch-to-buffer source)
          (insert "Work that must not be replaced by closing a media frame.")
          (video-open (video-test--fixture) :buffer viewer)
          (setq-local video-quit-function (lambda () (setq quit-called t)))
          (let* ((origin (selected-window))
                 (player video--buffer-player)
                 (deadline (+ (float-time) 5))
                 (sibling (split-window-right))
                 original-view sibling-view)
            (while (and (not (video-player-seekable player))
                        (< (float-time) deadline))
              (accept-process-output nil 0.05))
            (should (video-player-seekable player))
            (video-player-pause player)
            (video--manage-window-targets)
            (with-selected-window sibling
              (video-set-scale 150)
              (setq sibling-view (copy-video--view (video--window-view))))
            (video-set-scale 200)
            (setq original-view (copy-video--view (video--window-view)))
            (video-toggle-frame)
            (setq detached (selected-frame))
            (should (eq (window-buffer origin) viewer))
            (video-set-scale 350)
            (video-pan-left)
            (let ((detached-view (copy-video--view (video--window-view))))
              (select-window origin)
              (video-toggle-frame)
              (should (eq (selected-frame) detached))
              (should (equal (video--window-view) detached-view)))
            (should (equal (video--window-view origin) original-view))
            (should (equal (video--window-view sibling) sibling-view))
            ;; Closing the frame must not restore a video over new work.
            (set-window-buffer origin source)
            (delete-frame detached)
            (should (eq (window-buffer origin) source))
            (should (equal (video--window-view sibling) sibling-view))
            (should (video-player-live-p player))
            ;; Reopening has exactly the same additive behavior.
            ...
            (let ((other-player video--buffer-player)
                  (other-view (copy-video--view (video--window-view origin))))
              (select-frame-set-input-focus detached)
              (video-quit)
              (should-not (frame-live-p detached))
              (should-not quit-called)
              (should (buffer-live-p viewer))
              (should (eq (window-buffer origin) other))
              (should (eq (buffer-local-value 'video--buffer-player other) other-player))
              (should (equal (video--window-view origin) other-view))
              (should (equal (video--window-view sibling) sibling-view)))
            (select-window sibling)
            (video-quit)
            (should quit-called)))
      (when (frame-live-p detached)
        (set-frame-parameter detached 'video-presentation-buffer nil)
        (delete-frame detached))
  ...
  (let ((canvas `(image :type canvas :id ,(gensym "video-scene-test-")
    ...
    (should
     (video-native-canvas-draw-uri
      canvas 200 120
      (video-source-uri (video-test--fixture))
      20 15 160 90 "cover"))
    (should
     (video-native-canvas-draw-uri
      canvas 200 120
...
(ert-deftest video-presentation-refuses-to-overwrite-file-buffer ()
  (with-temp-buffer
    ...
    ;; Restore the temporary buffer's non-file identity before its cleanup.
    (let* ((buffer-file-name "/virtual/image.png")
    ...
    (let ((old (generate-new-buffer " *video-old-image*"))
      ...
      (unwind-protect
          (cl-letf (((symbol-function 'video-native-target-close) #'ignore)
                    ((symbol-function 'video-native-target-create)
                     (lambda (&rest _) 'new))
                    ((symbol-function 'video-native-target-copy)
                     (lambda (&rest _) sequence))
                    ((symbol-function 'video-canvas-create)
                     (lambda (&rest _) new-image))
            ...
            (setq new-target (video--create-window-target (selected-window)))
            (let ((overlay (video--window-overlay (selected-window))))
